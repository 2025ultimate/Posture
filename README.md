...
- **Check** — measurable check-ins, three ways. A **live standing coach**: stand sideways to the camera and it *talks you into a neutral stack*, one spoken correction at a time in physio order — soften the knees, hips back over ankles, ribs down, chin back — praises each fix, tells you when you've hit neutral ("take three breaths and memorize how this feels"), catches you when you drift, and can save the moment as a check-in. A guided **side-view snapshot** (prop your phone, countdown, auto-capture) for tracked measurements over weeks. And the two classic at-home self-tests, guided step by step: the **wall test** (lumbar gap) and the **Thomas test** (hip flexor length).
...
- **Learn** — plain-language education: what APT is (and how much tilt is normal), why sitting feeds it, what fixes it, what cameras can and can't measure, and when to see a professional. Plus a searchable **library of 876 exercises** (public-domain [Free Exercise DB](https://github.com/yuhonas/free-exercise-db)) with photos and instructions, filtered to APT-relevant muscles by default — for variety beyond the curated program. The routine player also offers **weight × reps logging** on loaded strength exercises, with a progression view in Progress.
